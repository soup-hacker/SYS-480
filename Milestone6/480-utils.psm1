...
function NewPortGroup ($config){
    ...
    if($found){
        ...
        try {
            $selected_switch = $switches[$pick_index -1]
...
function NetworkChange($config){
    ...
    Get-VM $selected_vm | Get-NetworkAdapter -Name "Network adapter $adapter_select" | Set-NetworkAdapter -NetworkName $network -Confirm:$false
